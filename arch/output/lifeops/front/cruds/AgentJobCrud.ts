...
{name:'agent',type:'object',label:'agent',default:{"systemPrompt":"''","allowedTools":"[]"},groupTab: 'AGENT',objectFields: [{name:'systemPrompt',type:'longString',label:'systemPrompt',default:''},
{name:'allowedTools',type:'array.string',label:'allowedTools',default:[]}]},
{name:'schedule',type:'object',label:'schedule',default:{"type":null,"timezone":"'America/Argentina/Buenos_Aires'","runAt":null,"time":"''","daysOfWeek":"[]","daysOfMonth":"[]","interval":"{\"every\":null,\"unit\":null}","cronExpression":"''"},groupTab: 'SCHEDULE',objectFields: [{name:'type',type:'enum',label:'type',default:null,enum: ['once', 'daily', 'weekly', 'monthly', 'interval', 'cron']},
...
{name:'daysOfWeek',type:'array.enum',label:'daysOfWeek',default:[],enum: ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']},
{name:'daysOfMonth',type:'array.number',label:'daysOfMonth',default:[]},
{name:'interval',type:'object',label:'interval',default:{"every":null,"unit":null},objectFields: [{name:'every',type:'number',label:'every',default:null},
...
{name:'execution',type:'object',label:'execution',default:{"timeoutSeconds":300,"maxRetries":0},groupTab: 'EXECUTION',objectFields: [{name:'timeoutSeconds',type:'number',label:'timeoutSeconds',default:300},
{name:'maxRetries',type:'number',label:'maxRetries',default:0}]},
{name:'runtime',type:'object',label:'runtime',default:{"lastRunAt":null,"nextRunAt":null,"lastStatus":null},groupTab: 'RUNTIME',objectFields: [{name:'lastRunAt',type:'date',label:'lastRunAt',default:null},
{name:'nextRunAt',type:'date',label:'nextRunAt',default:null},
{name:'lastStatus',type:'enum',label:'lastStatus',default:null,enum: ['success', 'failed', 'timeout']}]},
{name:'createdBy',type:'ref',label:'createdBy',default:null,groupTab: 'GENERAL',ref: 'User',refDisplay: 'username'}
    ]
  }
