...
const AgentJobBaseSchema = z.object({
    ...
    interval: z.object({    every: z.number().min(1).nullable().optional(),
    unit: z.enum(['minutes', 'hours', 'days']).optional()}).optional(),
    cronExpression: z.string().optional()}),
    execution: z.object({    timeoutSeconds: z.number().nullable().optional().default(300),
    maxRetries: z.number().nullable().optional().default(0)}).optional().default({ timeoutSeconds: 300, maxRetries: 0 }),
    runtime: z.object({    lastRunAt: z.coerce.date().nullable().optional(),
    nextRunAt: z.coerce.date().nullable().optional(),
    lastStatus: z.enum(['success', 'failed', 'timeout']).optional()}).optional(),
